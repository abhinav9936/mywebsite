...
yellow_strip=pygame.image.load("yellowstrip.jpg")
...
def countdown_background():
    ...
    gamedisplays.blit(backgroundpic,(0,150))
    gamedisplays.blit(backgroundpic,(720,0))
    gamedisplays.blit(backgroundpic,(720,150))
    ...
    gamedisplays.blit(yellow_strip,(400,700))
    gamedisplays.blit(strip,(85,0))
    gamedisplays.blit(strip,(85,100))
    gamedisplays.blit(strip,(85,300))
    gamedisplays.blit(strip,(710,0))
    gamedisplays.blit(strip,(710,100))
    gamedisplays.blit(strip,(710,300))
...
def introduction():
    ...
    while introduction:
        ...
        textSurf,textRect=text_objects("This is an car game in which you need to dodge the cars",smalltext)
        textRect.center=(350,200)
        TextSurf,TextRect=text_objects("INSTRUCTION",largetext)
        TextRect.center=(400,100)
        gamedisplays.blit(TextSurf,TextRect)
        gamedisplays.blit(textSurf,textRect)
        ltextSurf,ltextRect=text_objects("ARROW LEFT : LEFT TURN",smalltext)
        ltextRect.center=(150,400)
        rtextSurf,rtextRect=text_objects("ARROW RIGHT : RIGHT TURN",smalltext)
...
def obstacle(obs_startx,obs_starty,obs):
    if obs==0:
        obs_pick1=pygame.image.load("car1.jpg")
    elif obs==1:
        obs_pick1=pygame.image.load("car2.jpg")
    elif obs==2:
        obs_pick1=pygame.image.load("car3.jpg")
    gamedisplays.blit(obs_pick1,(obs_startx,obs_starty))
...
def game_loop():
    ...
    obs_startx2=random.randrange(200,(display_width-200))
    obs_starty=-750
    obs_starty2=-1000
    obs_width = 21
    obs_height = 42
    ...
    while not bumped :
        ...
        if rel_y<800:
                ...
                gamedisplays.blit(yellow_strip,(400,rel_y+700))
        y2+= obstacle_speed  
        background()
        while (obs_startx==obs_startx2 and obs_starty==obs_starty2) :
            obs_startx2=obs_startx2=random.randrange(200,(display_width-200))
        obstacle(obs_startx,obs_starty,obs)
        obstacle(obs_startx2,obs_starty2,obs2)
        obs_starty+=obstacle_speed
        obs_starty2=obs_starty2+obstacle_speed+1
        car(x,y)
        score_system(passed,score)
        if x>715-car_width or x <85:
            crash()
        if x>display_width-(car_width+85) or x<85:
            crash()
        if obs_starty>display_height or obs_starty2>display_height:
            if obs_starty>display_height:
                obs_starty=0-obs_height
                obs_startx=random.randrange(170,(display_width-170))
                obs=random.randrange(0,3)
                passed=passed+1
            elif obs_starty2>display_height:
                obs_starty2=0-obs_height+10
                obs_startx2=random.randrange(200,(display_width-200))
                obs2=random.randrange(0,3)
                passed=passed+1
            score=passed*(10+level)
            if int(passed)%20==0:
                level=level+1
                obstacle_speed+=1
                largetext=pygame.font.Font("freesansbold.ttf",80)
        ...
        if y<obs_starty2+obs_height:
             if x > obs_startx2 and x < obs_startx2 + obs_width or x+car_width > obs_startx2 and x+car_width < obs_startx2+obs_width:
                crash()
